...
if ! grep -q "<length>12267</length>" dot-onetime/pad-records; then
  echo ""
...
if ! grep -q "<length>24593</length>" dot-onetime/pad-records; then
  echo ""
...
if ! grep -q "<length>37127</length>" dot-onetime/pad-records; then
  echo ""
...
if ! grep -q "<length>12452</length></used>" v1-dot-onetime/pad-records
then
  echo ""
  echo "ERROR: failed to insert expected new length 12452 into pad-records"
  cat v1-dot-onetime/pad-records
  PASSED="no"
elif grep -q "<length>12452</length></used>" v1-dot-onetime/pad-records && \
     [ `grep -n "<length>12452</length></used>" v1-dot-onetime/pad-records \
        | cut -d ":" -f 1` -ne 7 ]
then
  echo ""
